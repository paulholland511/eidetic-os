...
install an entire workflow with one command (``eidetic skills install-pack
...
(:func:`eidetic_os._skills.install_skill`) for every member, with the same
...
def install_pack(
    ...
    :class:`eidetic_os._skills.SkillInstallError` when there's no install target at
    ...
    if skills_install_root(env) is None:
        raise SkillInstallError(
            "no install target — set EIDETIC_SKILLS_DIR or VAULT_PATH (run `eidetic init`)"
        )
    ...
    for slug in pack.skills:
        try:
            ...
            skipped.append((slug, "unknown skill — not in this Eidetic OS install"))
