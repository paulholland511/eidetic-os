"""Pluggable vector-storage backends for the Eidetic OS RAG pipeline.

The RAG store began life as a single SQLite database (:mod:`eidetic_os.vectordb`):
...
lazily and depend on extras (``eidetic-os[lancedb]`` / ``eidetic-os[chroma]``), so
...
class VectorBackend(ABC):
    ...
    #: override this so :func:`active_backend_name` and ``eidetic doctor`` can
...
def active_backend_name() -> str:
    ...
    Convenience for display surfaces (``eidetic doctor``) that want to report the
