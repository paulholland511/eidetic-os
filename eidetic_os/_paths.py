...
2. **Wheel / ``uv tool install`` / ``pipx install``** — the dirs are bundled
   into a top-level ``eidetic_os_data/`` directory on ``sys.path`` (deliberately
   *not* inside the ``eidetic_os`` package, so it can never shadow the import).
"""
...
def _data_root() -> Path | None:
    """The bundled eidetic_os_data dir from a wheel install, if present."""
...
def _resolve(name: str) -> Path:
    ...
    raise FileNotFoundError(
        ...
        "Reinstall Eidetic OS, or run from a source checkout."
    )
