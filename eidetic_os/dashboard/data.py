...
Every function here reads live state from the existing Eidetic OS modules and
...
from eidetic_os._paths import scripts_dir, skills_dir
...
# ── 1. system health (eidetic doctor) ────────────────────────────────────────────
def health_report() -> dict[str, Any]:
    """Run the doctor checks and group them for display.

    Reuses ``eidetic doctor``'s own check logic (``cli._doctor_results``) so the
...
def audit_page(
    ...
    ``eidetic audit show --since`` (``24h``, ``7d``, ``2026-06-01``). An invalid
...
def install_pack(name: str, *, force: bool = False) -> dict[str, Any]:
    ...
    Wraps :func:`eidetic_os.packs.install_pack`, turning its result (or the
...
def vector_stats() -> dict[str, Any]:
    ...
    amber "no index yet" panel with a hint to run ``eidetic embed --full``.
    """
    ...
    if not db_path.exists():
        return {
            "available": False,
            "reason": "no vectors yet — run `eidetic embed --full` to build the index.",
            "rag_dir": str(rag_dir),
        }
...
def run_search(query: str, top_k: int = 5, mode: str = "hybrid") -> dict[str, Any]:
    """Run ``eidetic search`` for ``query`` and return parsed results.

    Shells out to the same ``scripts/rag_search.py`` the ``eidetic search`` command
...
# without a prior ``eidetic graph`` run and the function stays unit-testable.
_WIKILINK_RE = re.compile(r"\[\[([^\]]+)\]\]")
_GRAPH_SKIP_DIRS = frozenset({
    ".obsidian", ".git", ".rag", ".claude", ".eidetic", "node_modules", ".schemas",
})
