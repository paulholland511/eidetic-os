...
folder (see :mod:`eidetic_os._skills`). The marketplace adds the plumbing to turn
...
:mod:`eidetic_os.netio` (for URL fetches) so the CLI layer stays thin and the
...
from eidetic_os._paths import skills_dir
from eidetic_os._skills import _parse_frontmatter, find_skill
...
def registries_path(env: dict[str, str] | None = None) -> Path:
    ...
    Order: ``EIDETIC_REGISTRIES_PATH`` → ``$VAULT_PATH/.eidetic/registries.json`` →
    ``./.eidetic/registries.json`` — mirroring how the audit log is located.
    """
    environ = os.environ if env is None else env
    override = environ.get("EIDETIC_REGISTRIES_PATH")
    if override:
        return Path(os.path.expanduser(override))
    vault = environ.get("VAULT_PATH")
    base = Path(os.path.expanduser(vault)) if vault else Path.cwd()
    return base / ".eidetic" / "registries.json"
