"""A SQLite-backed vector store for the Eidetic OS RAG pipeline.
...
def _try_load_vec(conn: sqlite3.Connection) -> bool:
    ...
    ``EIDETIC_VECTORDB_NO_VEC=1`` forces the brute-force backend regardless. Either
    way we degrade to the brute-force backend rather than failing.
    """
    if os.environ.get("EIDETIC_VECTORDB_NO_VEC"):
        return False
