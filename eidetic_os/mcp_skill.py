"""Wrap Eidetic OS ``SKILL.md`` skills as MCP tools — no skill changes required.

Eidetic skills are prompt-based: a ``SKILL.md`` is a YAML-frontmatter header (name,
...
* :func:`skill_to_tool` projects one :class:`~eidetic_os._skills.Skill` into an MCP
  :class:`~eidetic_os.mcp_server.Tool`. Calling the tool renders the skill's
...
* :func:`build_skill_server` auto-generates an MCP server shim exposing every
  ...
  is what ``eidetic skills run <name>`` launches.
...
from eidetic_os._skills import (
    Skill,
    _parse_frontmatter,
    load_skills,
    skill_source,
...
from eidetic_os.mcp_server import MCPServer, Tool

# MCP tool names must be a safe slug; Eidetic skill slugs already are ([a-z0-9-]).
...
def build_skill_server(slugs: Sequence[str] | None = None) -> MCPServer:
    ...
    server (``eidetic skills run <name>``) from the full catalog.
    """
    ...
    if slugs is None:
        ...
        for slug in slugs:
            ...
            chosen.append(skill)
        name = f"eidetic-skill-{slugs[0]}" if len(slugs) == 1 else "eidetic-skills"
...
def serve_skill(slug: str) -> None:
    """Entry point for ``eidetic skills run <name>`` — serve one skill over stdio."""
