"""Append-only audit trail for every autonomous Atlas OS action.

Every command Atlas runs on your behalf — embedding the vault, committing to
...
The log lives at ``$ATLAS_AUDIT_PATH`` if set, otherwise
``$VAULT_PATH/.atlas/audit.jsonl``, otherwise ``./.atlas/audit.jsonl``.
...
``atlas`` processes never interleave a line.
...
def audit_path() -> Path:
    ...
    Order: ``ATLAS_AUDIT_PATH`` → ``VAULT_PATH/.atlas/audit.jsonl`` →
    ``./.atlas/audit.jsonl``. Read fresh each call so tests and scheduled jobs
    can redirect it via the environment.
    """
    override = os.environ.get("ATLAS_AUDIT_PATH")
    if override:
        return Path(os.path.expanduser(override))
    vault = os.environ.get("VAULT_PATH")
    base = Path(os.path.expanduser(vault)) if vault else Path.cwd()
    return base / ".atlas" / "audit.jsonl"
