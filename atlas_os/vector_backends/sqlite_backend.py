...
This is a thin :class:`~atlas_os.vector_backend.VectorBackend` adapter over the
existing :class:`atlas_os.vectordb.VectorStore`. It adds no new storage code: it
...
behaves exactly as Atlas OS always has. The store accelerates search with the
``sqlite-vec`` extension when present and falls back to a NumPy/pure-Python
cosine scan otherwise — see :mod:`atlas_os.vectordb`.
"""
...
from atlas_os.vector_backend import SQLITE, VectorBackend
from atlas_os.vectordb import VectorStore, open_store
...
class SQLiteBackend(VectorBackend):
    ...
    def open(cls, rag_dir: str | Path) -> SQLiteBackend:
        ...
        Delegates to :func:`atlas_os.vectordb.open_store`, which imports an
